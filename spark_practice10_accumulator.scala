...
val words = sc.textFile("/Users/spark/spark/README.md").flatMap(line => line.split(' '))
...
//accumulator update는 map 대신 foreach를 사용해야함. 정확성을 위해. action(즉시 실행되는)으로 side effect 유발하는 함수는 foreach 사용
